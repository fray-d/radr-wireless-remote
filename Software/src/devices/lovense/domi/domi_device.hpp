...
class Domi2 : public Device {
  public:
    ...
    void drawControls() override {
        ...
        std::map<String, float *> leftParams = {{"Vibrate", &this->vibrateIntensity}};
        draw<EncoderDial>(EncoderDial::Props{
            .encoder = &leftEncoder, .parameters = leftParams, .focusedIndex = &this->leftFocusedIndex, .minValue = 0, .maxValue = 16});

        draw<TextButton>("STOP", pins::BTN_UNDER_C, DISPLAY_WIDTH / 2 - 60, DISPLAY_HEIGHT - 30, 120);
    }
